...
def multiple_hists(df_map, histnorm='probability', title=''):
    ...
    all_sets = all_sets.reset_index()[['level_0', 'child']].rename(columns={'level_0': 'dataset'})
    fig = px.histogram(all_sets, color='dataset', x='child', barmode='overlay', histnorm=histnorm)
...
def multiple_describe(df_map):
    out = pd.DataFrame(columns=['Dataset', 'Mean', 'Standard Deviation']).set_index('Dataset')
    for key in df_map:
